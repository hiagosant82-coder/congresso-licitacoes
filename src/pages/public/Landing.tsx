...
import { initMetaPixel, fireMetaPurchase } from '../../lib/tracking'
...
const Navbar = () => {
  ...
  const clientAreaLink = user
    ? user.role === 'admin' || user.role === 'operator'
      ? '/admin'
      : '/app/dashboard'
    : '/login'

  const clientAreaLabel = user ? 'Painel' : 'Área do Cliente'
    ...
    <nav className="fixed top-0 left-0 right-0 bg-[#0A0706]/90 backdrop-blur-md z-50 py-4 border-b border-amber-500/10">
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-6">
          <a href="#" className="text-stone-400 font-bold text-xs hover:text-amber-400 transition-all uppercase tracking-widest">Home</a>
          <a href="#palestrantes" className="text-stone-400 font-bold text-xs hover:text-amber-400 transition-all uppercase tracking-widest">Palestrantes</a>
          <a href="#programacao" className="text-stone-400 font-bold text-xs hover:text-amber-400 transition-all uppercase tracking-widest">Programação</a>
          <a href="#contato" className="text-stone-400 font-bold text-xs hover:text-amber-400 transition-all uppercase tracking-widest">Contato</a>
          <a href="#inscricao" className="bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-[#0E0B08] px-6 py-2.5 rounded-full font-bold text-xs shadow-[0_0_15px_rgba(245,158,11,0.3)] hover:shadow-[0_0_25px_rgba(245,158,11,0.5)] transition-all uppercase tracking-widest duration-300">
            Inscrever-se
          </a>
          <Link to={clientAreaLink} className="bg-[#16120F] border border-amber-500/30 text-amber-400 hover:bg-amber-500 hover:text-[#0E0B08] px-6 py-2.5 rounded-full font-bold text-xs transition-all uppercase tracking-widest duration-300">
            {clientAreaLabel}
          </Link>
        </div>
        ...
        {isOpen && (
          <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="md:hidden bg-[#0A0706] border-t border-amber-500/10 px-6 overflow-hidden">
            <div className="flex flex-col gap-4 py-6">
              ...
              <a href="#inscricao" className="bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-[#0E0B08] py-3 rounded-xl font-bold uppercase tracking-widest text-center shadow-[0_0_15px_rgba(245,158,11,0.2)]" onClick={() => setIsOpen(false)}>Inscrever-se</a>
              <Link to={clientAreaLink} className="bg-[#16120F] border border-amber-500/30 text-amber-400 hover:bg-amber-500 hover:text-[#0E0B08] py-3 rounded-xl font-bold uppercase tracking-widest text-center" onClick={() => setIsOpen(false)}>
                {clientAreaLabel}
              </Link>
            </div>
          </motion.div>
...
const registrationSchema = z.object({
  ...
  position: z.string().min(2, 'Informe seu cargo'),
  workplace: z.string().min(2, 'Informe onde trabalha'),
})
...
export default function Landing() {
  ...
  const onRegister = async (data: RegistrationForm) => {
    ...
    let orgId = import.meta.env.VITE_ORGANIZATION_ID

    if (!orgId) {
      const orgName = import.meta.env.VITE_ORGANIZATION_NAME
      if (orgName) {
    ...
    if (!orgId) {
      setFormError('Configuração da organização não encontrada. Contate o suporte.')
    ...
    if (error) {
      setFormError(error.message || 'Erro ao enviar inscrição. Tente novamente.')
    } else {
    ...
    <div className="min-h-screen bg-[#0A0706] font-sans text-stone-200 selection:bg-amber-500/20 selection:text-amber-100">
      ...
      <section className="py-16 md:py-24 px-4 overflow-hidden bg-transparent">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
          <div className="relative w-full max-w-md aspect-square shrink-0">
            <div className="absolute inset-0 border border-dashed border-amber-500/10 rounded-full" />
            <div className="absolute inset-14 border border-dashed border-amber-500/5 rounded-full" />
            <div className="absolute inset-28 border border-dashed border-amber-500/3 rounded-full" />
          ...
          <div className="space-y-10 flex-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-8">
              {[
                ...
                { title: 'Certificação', desc: 'Certificado de 40h de participação no evento.' },
                { title: 'Conteúdo Exclusivo', desc: 'Acesso a materiais e apresentações dos palestrantes.' },
            ...
            {[
              ...
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab.id
                    ? 'bg-gradient-to-r from-amber-500 to-amber-600 text-[#0E0B08] shadow-[0_4px_15px_rgba(245,158,11,0.25)]'
      ...
      <section className="py-16 md:py-24 bg-gradient-to-b from-[#0A0706] to-[#14100E] px-6">
        <div className="max-w-5xl mx-auto text-center space-y-8 md:space-y-12">
          <h2 className="text-2xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-100 via-amber-300 to-amber-400">Um convite especial para você</h2>
          ...
          <div className="relative aspect-video rounded-3xl md:rounded-[3rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.8)] bg-black/60 border border-amber-500/15 group">
            ...
            <div className="absolute bottom-8 left-8 right-8 flex items-center gap-6 text-white/90">
              ...
              <div className="flex-1 h-1 bg-white/20 rounded-full overflow-hidden">
                <div className="w-1/3 h-full bg-gradient-to-r from-amber-400 to-amber-500" />
              </div>
      ...
      <section id="inscricao" className="py-16 md:py-24 px-4 md:px-6 bg-gradient-to-b from-[#0A0706] to-[#120E0B]">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row rounded-3xl lg:rounded-[4rem] bg-gradient-to-br from-[#120F0D] via-[#1A1512] to-[#0D0B09] border border-amber-500/15 shadow-[0_20px_50px_rgba(0,0,0,0.8)] overflow-hidden">
          <div className="lg:w-[450px] bg-gradient-to-b from-[#1C1613] to-[#110D0B] p-8 md:p-16 text-stone-200 space-y-8 md:space-y-12 relative border-r border-amber-500/10">
            <div className="absolute top-0 right-0 w-full h-full opacity-10 pointer-events-none overflow-hidden">
              <div className="absolute -top-20 -right-20 w-80 h-80 border-[40px] border-amber-500/10 rounded-full" />
            </div>
            ...
            <div className="pt-10 border-t border-amber-500/10">
              <div className="w-16 h-16 border-4 border-amber-500 rounded-full flex items-center justify-center">
                <div className="w-8 h-8 border-2 border-amber-500/20 rotate-45" />
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 p-6 md:p-20 space-y-8 md:space-y-12">
            ...
            <form className="grid grid-cols-1 md:grid-cols-2 gap-8" onSubmit={handleSubmit(onRegister)}>
              {formSuccess && (
                <div className="md:col-span-2 p-4 bg-emerald-950/30 border border-emerald-500/30 rounded-xl text-emerald-400 text-sm text-center">
                  Inscrição enviada com sucesso! Entraremos em contato em breve.
                </div>
              )}
              {formError && (
                <div className="md:col-span-2 p-4 bg-red-950/30 border border-red-500/30 rounded-xl text-red-400 text-sm text-center">
                  {formError}
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <input type="text" placeholder="Seu nome" {...register('full_name')} className="w-full bg-[#0A0706] border border-stone-850 focus:border-amber-500/60 rounded-xl px-6 py-4 focus:ring-1 focus:ring-amber-500/40 outline-none font-bold text-stone-200 text-sm placeholder-stone-700 transition-all duration-300" />
              ...
              <div className="space-y-2">
                ...
                <input type="text" placeholder="000.000.000-00" {...register('cpf')} className="w-full bg-[#0A0706] border border-stone-850 focus:border-amber-500/60 rounded-xl px-6 py-4 focus:ring-1 focus:ring-amber-500/40 outline-none font-bold text-stone-200 text-sm placeholder-stone-700 transition-all duration-300" />
                {errors.cpf && <p className="text-xs text-red-400 ml-1">{errors.cpf.message}</p>}
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-amber-200/50 uppercase tracking-widest ml-1">E-mail*</label>
                <input type="email" placeholder="seu@email.com" {...register('email')} className="w-full bg-[#0A0706] border border-stone-850 focus:border-amber-500/60 rounded-xl px-6 py-4 focus:ring-1 focus:ring-amber-500/40 outline-none font-bold text-stone-200 text-sm placeholder-stone-700 transition-all duration-300" />
                {errors.email && <p className="text-xs text-red-400 ml-1">{errors.email.message}</p>}
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-amber-200/50 uppercase tracking-widest ml-1">WhatsApp*</label>
                <input type="tel" placeholder="(00) 00000-0000" {...register('phone')} className="w-full bg-[#0A0706] border border-stone-850 focus:border-amber-500/60 rounded-xl px-6 py-4 focus:ring-1 focus:ring-amber-500/40 outline-none font-bold text-stone-200 text-sm placeholder-stone-700 transition-all duration-300" />
                {errors.phone && <p className="text-xs text-red-400 ml-1">{errors.phone.message}</p>}
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-amber-200/50 uppercase tracking-widest ml-1">Cargo*</label>
                <input type="text" placeholder="Ex: Secretário" {...register('position')} className="w-full bg-[#0A0706] border border-stone-850 focus:border-amber-500/60 rounded-xl px-6 py-4 focus:ring-1 focus:ring-amber-500/40 outline-none font-bold text-stone-200 text-sm placeholder-stone-700 transition-all duration-300" />
                {errors.position && <p className="text-xs text-red-400 ml-1">{errors.position.message}</p>}
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-amber-200/50 uppercase tracking-widest ml-1">Onde trabalha?*</label>
                <input type="text" placeholder="Ex: Prefeitura de Barra do Garças" {...register('workplace')} className="w-full bg-[#0A0706] border border-stone-850 focus:border-amber-500/60 rounded-xl px-6 py-4 focus:ring-1 focus:ring-amber-500/40 outline-none font-bold text-stone-200 text-sm placeholder-stone-700 transition-all duration-300" />
                {errors.workplace && <p className="text-xs text-red-400 ml-1">{errors.workplace.message}</p>}
              </div>

              <button type="submit" disabled={formSubmitting} className="md:col-span-2 bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-[#0E0B08] py-6 rounded-2xl font-black text-xl shadow-[0_4px_25px_rgba(245,158,11,0.3)] hover:shadow-[0_4px_35px_rgba(245,158,11,0.5)] transition-all duration-300 uppercase tracking-widest disabled:opacity-70 disabled:cursor-not-allowed">
                {formSubmitting ? <Loader2 className="w-6 h-6 animate-spin inline" /> : 'CONTINUAR'}
              </button>
            </form>
      ...
      <a
        href={`https://wa.me/${import.meta.env.VITE_WHATSAPP_NUMBER || '5500000000000'}?text=Olá! Tenho interesse no Congresso de Licitações.`}
        target="_blank"
        rel="noopener noreferrer"
        onClick={() => fireMetaPurchase(1997.90)}
        className="fixed bottom-6 right-6 z-50 bg-green-500 hover:bg-green-600 text-white p-4 rounded-full shadow-[0_4px_20px_rgba(34,197,94,0.4)] hover:shadow-[0_4px_30px_rgba(34,197,94,0.6)] transition-all duration-300 hover:scale-110"
        aria-label="WhatsApp"
      >
      ...
      <noscript>
        <img height="1" width="1" style={{ display: 'none' }} src={`https://www.facebook.com/tr?id=${import.meta.env.VITE_META_PIXEL_ID || ''}&ev=PageView&noscript=1`} alt="" />
      </noscript>
      ...
      <footer className="py-12 md:py-20 px-6 border-t border-stone-850/40 bg-[#070504]">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8 md:gap-12">
          ...
          <a href="#inscricao" className="bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-[#0E0B08] px-8 py-3 rounded-full font-black text-xs shadow-[0_0_15px_rgba(245,158,11,0.25)] transition-all uppercase tracking-widest text-center">
            INSCREVER-SE
          </a>
        </div>
      </footer>
        ...
        {formSuccess && registeredData && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-xl bg-gradient-to-br from-[#1E1713] via-[#120F0D] to-[#0A0706] border border-amber-500/20 rounded-[2.5rem] p-8 md:p-12 shadow-[0_20px_60px_rgba(0,0,0,0.8)] overflow-hidden text-center space-y-8"
            >
              ...
              <div className="flex justify-center">
                <motion.div
                  ...
                  className="w-20 h-20 rounded-full bg-[#D4AF37]/10 border-2 border-[#D4AF37]/40 flex items-center justify-center text-[#D4AF37] shadow-[0_0_30px_rgba(212,175,55,0.2)]"
                >
              ...
              <div className="space-y-3">
                <span className="text-[10px] font-black tracking-[0.3em] text-[#D4AF37] uppercase block">Inscrição Recebida</span>
                <h3 className="text-2xl md:text-3xl font-black text-[#F5E6C4] tracking-tight leading-tight">
                  Quase tudo pronto, <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-300 via-yellow-200 to-amber-500">{registeredData.full_name.split(' ')[0]}</span>! 🚀
                </h3>
                <p className="text-stone-400 text-xs md:text-sm font-medium">
                  Sua solicitação de vaga foi enviada com sucesso e já está disponível em nosso sistema CRM em tempo real.
                </p>
              </div>

              {/* Next Steps Visual Flow */}
              <div className="bg-[#0E0B0A]/80 border border-stone-850 rounded-2xl p-6 text-left space-y-4 relative">
                <p className="text-[9px] font-black text-amber-500/40 uppercase tracking-widest block mb-2">O QUE ACONTECE AGORA?</p>
                
                <div className="space-y-4">
                  <div className="flex gap-4">
                    <div className="w-6 h-6 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/35 text-[#D4AF37] flex items-center justify-center text-xs font-black shrink-0">1</div>
                    <div className="space-y-0.5">
                      <h4 className="text-xs font-black text-stone-200">Análise de Dados</h4>
                      <p className="text-[10px] text-stone-400 font-medium">Nosso time comercial recebeu seus dados de cadastro no CRM e está validando as informações neste instante.</p>
                    </div>
                  </div>

                  <div className="flex gap-4">
                    <div className="w-6 h-6 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/35 text-[#D4AF37] flex items-center justify-center text-xs font-black shrink-0">2</div>
                    <div className="space-y-0.5">
                      <h4 className="text-xs font-black text-stone-200">Contato pelo WhatsApp</h4>
                      <p className="text-[10px] text-stone-400 font-medium">Em alguns minutos, um de nossos especialistas em licitações entrará em contato com você pelo número <span className="text-amber-200 font-bold">{registeredData.phone}</span> para definir o método de pagamento ideal (Pix, Cartão ou Empenho Público) e tirar suas dúvidas.</p>
                    </div>
                  </div>

                  <div className="flex gap-4">
                    <div className="w-6 h-6 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/35 text-[#D4AF37] flex items-center justify-center text-xs font-black shrink-0">3</div>
                    <div className="space-y-0.5">
                      <h4 className="text-xs font-black text-stone-200">Acesso à Área de Membros</h4>
                      <p className="text-[10px] text-stone-400 font-medium">Com o pagamento confirmado, suas credenciais de login provisórias serão enviadas diretamente para seu e-mail <span className="text-amber-200 font-bold">{registeredData.email}</span>.</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <a
                  href="https://www.instagram.com/envolve.consultoria"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-[#0E0B08] py-4 rounded-xl font-black text-sm shadow-[0_4px_20px_rgba(245,158,11,0.25)] hover:shadow-[0_4px_30px_rgba(245,158,11,0.4)] transition-all duration-300 uppercase tracking-wider block text-center"
                >
                  📲 FALAR COM A EQUIPE NO INSTAGRAM
                </a>
                ...
                <button
                  ...
                  className="text-stone-500 hover:text-stone-300 font-bold text-xs uppercase tracking-widest py-2 transition-colors duration-300"
                >
                  Voltar ao site
                </button>
              </div>
